...
export default function AuditLogsPage() {
  ...
  const handleExportPdf = () => {
    const printContents = document.querySelector(".audit-logs-table")?.innerHTML
    ...
    win.document.write(`
      <html>
        <head>
          <title>Audit Logs - STTB CMS</title>
          <style>
        ...
        <body>
          <h2 style="margin-bottom:16px">Audit Logs — STTB CMS</h2>
          ${printContents}
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-500">
              {item.user?.email || "automated@sttb.ac.id"}
            </p>
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      render: (item) => (
        <div className="max-w-[200px]">
          <p className="text-xs font-semibold text-gray-700 uppercase tracking-tighter">
            {item.resourceType}
          </p>
          <p className="text-[10px] text-gray-400 truncate">
            ID: {item.resourceId || "N/A"}
          </p>
        </div>
      ),
    },
