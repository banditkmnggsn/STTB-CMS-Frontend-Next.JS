...
import { Plus, Search, Edit, Trash2, Loader2, Newspaper, BookOpen, Layers } from 'lucide-react';
...
export default function ContentListPage() {
  ...
  const getTypeIcon = (type: ContentType) => {
    ...
    if (type === 'articles') return <BookOpen size={18} />;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <PageHeader 
        ...
        description="Manage your News, Articles, and Programs in one place"
        breadcrumbs={[{ label: 'Content' }]}
        actions={
          <button 
            onClick={() => router.push(`/content/new?type=${activeType}`)}
            className="flex items-center gap-2 px-4 py-2 bg-[#C1121F] text-white rounded-lg hover:bg-[#9A0E19]"
          >
            <Plus size={18} /> New {activeType.slice(0, -1)}
          </button>
        }
      />
      ...
      <div className="p-8">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="flex border-b border-gray-200 bg-gray-50/50">
            {(['news', 'articles', 'programs'] as ContentType[]).map((type) => (
              <button
                key={type}
          ...
          <div className="p-4 border-b border-gray-200 flex justify-between items-center">
            <div className="relative w-72">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 border rounded-lg text-sm focus:ring-1 focus:ring-[#C1121F] outline-none"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
              />
            </div>
            <p className="text-xs text-gray-400 font-medium">Showing {items.length} items</p>
          </div>
                ...
                {isLoading ? (
                    ...
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {/* Preview Image dari Media Library */}
                          <div className="w-12 h-12 rounded-lg bg-gray-100 border overflow-hidden flex-shrink-0 relative">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button 
                            ...
                            className="p-2 text-gray-400 hover:text-[#C1121F] hover:bg-[#C1121F]/5 rounded-lg transition-all"
                          >
                            <Edit size={16} />
                          </button>
                          <button 
                  ...
                  <tr>
                    <td colSpan={4} className="py-20 text-center text-gray-400 text-sm">
                      No {activeType} found. Start by creating one!
                    </td>
                  </tr>
                )}
              </tbody>
